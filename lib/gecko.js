...
class GeckoDriverProcess {
  ...
  async init () {
      ...
      await GECKO_SERVER_GUARD(async () => {
        ...
        try {
          ...
          throw new Error(
            ...
            `these which are not needed anymore or set any free port number to the 'port' capability`);
        }
      });
    }
    ...
    if (_.toLower(this.verbosity) === VERBOSITY.DEBUG) {
      args.push('-v');
    } else if (_.toLower(this.verbosity) === VERBOSITY.TRACE) {
      args.push('-vv');
    }
    if (this.noReset) {
...
process.once('exit', () => {
  for (const pid of RUNNING_PROCESS_IDS) {
    const command = system.isWindows() ? `taskkill.exe /PID ${pid} /F` : `kill ${pid}`;
    try {
      execSync(command);
    } catch (ign) {}
  }

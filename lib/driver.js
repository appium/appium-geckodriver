...
export class GeckoDriver extends BaseDriver {
  ...
  async createSession (...args) {
    ...
    this._bidiProxyUrl = this._processResponseIfBiDi(response);
  ...
  /**
   *
   * @param {import('@appium/types').StringRecord | null} response
   * @returns {string | null}
   */
  _processResponseIfBiDi(response) {
    this.log.info(`Create session reponse: ${JSON.stringify(response)}`);
    ...
    return asUrl.host !== GECKO_SERVER_HOST

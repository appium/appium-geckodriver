...
const DEFAULT_INIT_URL = 'https://www.mozilla.org/';
...
class Geckodriver extends events.EventEmitter {
  ...
  async start (caps, emitStartingState = true) {
    ...
    try {
      ...
      log.info(`Spawning geckodriver with: ${this.geckodriver} ` +
              `${args.join(' ')}. Only geckodriver 0.26.0 supports Android testing`);
  ...
  prepareMozCapabilities () {
    let mozFirefoxOptions = {
      ...
      androidIntentArguments: [
        '-d', this.capabilities.firefoxInitialUrl || DEFAULT_INIT_URL
      ]
    };
    if (!_.has(this.capabilities, 'capabilities.alwaysMatch["moz:firefoxOptions"]') === true) {
      this.capabilities.capabilities.alwaysMatch['moz:firefoxOptions'] = {};
    }
    _.extend(this.capabilities.capabilities.alwaysMatch['moz:firefoxOptions'], mozFirefoxOptions);
    delete this.capabilities.capabilities.firefoxInitialUrl;
  }
  ...
  async startSession () {
    ...
    await this.jwproxy.command('/session', 'POST', this.capabilities);
  }

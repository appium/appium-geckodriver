...
const DEFAULT_SYSTEM_GECKODRIVER = '/usr/local/bin/geckodriver';
...
class Geckodriver extends events.EventEmitter {
  constructor (args = {}) {
    ...
    const {
      host = DEFAULT_HOST,
      port = DEFAULT_PORT,
      geckodriverExecutable = DEFAULT_SYSTEM_GECKODRIVER,
      noReset = false,
  ...
  getVersion = _.memoize(async () => {
    const proc = new SubProcess(this.geckodriverExecutable, ['--version']);
    let version;
  ...
  async start (caps, emitStartingState = true) {
    ...
    try {
      this.proc = new SubProcess(this.geckodriverExecutable, args);
      processIsAlive = true;
      ...
      log.info(`Spawning geckodriver with: ${quote([this.geckodriverExecutable, ...args])}`);
  ...
  async deleteSession () {
    await this.jwproxy.command('/shutdown', 'GET', {});
  }

...
class Geckodriver extends events.EventEmitter {
  ...
  prepareMozCapabilities () {
    let caps = this.capabilities.alwaysMatch || this.capabilities.firstMatch || this.capabilities;
    let mozFirefoxOptions = {
      log: {
        level: this.logLevel
      },
      androidPackage: caps['moz:firefoxOptions'].androidPackage || FIREFOX_DEFAULT_PACKAGE,
      androidActivity: caps['moz:firefoxOptions'].androidActivity || FIREFOX_DEFAULT_MAIN_ACTIVITY
    };
